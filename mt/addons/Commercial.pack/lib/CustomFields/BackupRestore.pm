...
#
# $Id: BackupRestore.pm 70810 2008-01-08 22:58:00Z bchoate $
...
sub cb_restore_objects {
    ...
    for my $placement ( values %placements ) {
        ...
        unless ( exists $class_fields{ $class_type } ) {
            ...
            while ( my $field = $iter->() ) {
                ...
                $class_fields{ $field->obj_type } = []
                    unless exists $class_fields{ $field->obj_type };

                push @{ $class_fields{ $field->obj_type } }, $field->basename;
            }
        }
        ...
        for my $basename ( @{ $class_fields{ $class_type } } ) {
