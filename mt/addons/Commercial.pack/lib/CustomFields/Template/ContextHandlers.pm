...
sub _hdlr_customfield_basename {
    my $field = $_[1]->stash('field')
        or return _no_field($_[1]);
...
sub _hdlr_customfield_value {
    ...
    my $obj;
    eval { $obj = &_hdlr_customfield_obj($plugin, $ctx, $field->obj_type); };
    return $ctx->error($@) if $@;
    ...
    if(defined $value) {
        ...
        if($fld_type eq 'textarea') {
            ...
            $value = MT::Template::Context::_hdlr_date($ctx, $args);
        }
        $res = $value;
    }
...
sub _hdlr_customfield_asset {
    ...
    my $value = _hdlr_customfield_value($plugin, $ctx, $args);
...
sub _hdlr_object_blocks {
    my $plugin = shift;
    my ($ctx, $args, $cond, $obj_type, $method) = @_;

    my $class = MT->model($obj_type);
    my $hdlr_method = $plugin->{"${method}_method"};
    my $sort_by = $args->{sort_by};

    if(!$sort_by || $class->can($sort_by)) {
        # Use the normal MT::Entries if there isn't a sort_by or if the column exists in the table
        return &{$hdlr_method}(@_);
    }
    # Bug reports for v2.0RC suggested that custom sorting wasn't working properly
    # After some thought, it was obvious - the sorting occured *after* the entries had been loaded
    # less than ideal. Hence, now it adds a routine to the class ("symbol table manipulation" is apparently the fancy name)
    # and passes it back to $hdlr_method.
    no strict; 
    {
        *{"${class}::${sort_by}"} = sub {
            my $obj = shift;
