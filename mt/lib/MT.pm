...
#
# $Id: MT.pm.pre 1380 2008-02-20 20:46:49Z bchoate $
...
BEGIN {
    ...
    ( $VERSION, $SCHEMA_VERSION ) = ( '4.13', '4.0036' );
...
sub log_times {
    ...
    $log_file = File::Spec->catfile( $dir, $file );
...
sub get_timer {
    ...
    unless (defined $timer) {
        if (MT->config('PerformanceLogging')) {
            my $uri;
            if ($mt->isa('MT::App')) {
                $uri = $mt->uri( $mt->request_method eq 'GET' ? ( args => { $mt->param_hash } ) : () );
            }
...
sub build_page {
    ...
    $param->{portal_url} = $mt->translate("http://www.sixapart.jp/movabletype/");
