...
#
# $Id: Wizard.pm 83433 2008-06-18 21:44:24Z bchoate $
...
sub init_request {
    ...
    my $mode = $app->mode;
    return unless $mode eq 'previous_step' || $mode eq 'next_step'
        || $mode eq 'retry' || $mode eq 'test';
    ...
    $core->{registry}{applications}{wizard} = {
        wizard_steps => {
            ...
            configure => {
                order => 100,
                handler => \&configure,
                params => [qw(dbpath dbname dbport dbserver dbsocket
                    dbtype dbuser dbpass publish_charset)]
            },
            optional => {
                order => 200,
                handler => \&optional,
                params => [qw(mail_transfer sendmail_path smtp_server
                    test_mail_address)]
            },
            cfg_dir => {
                order => 300,
...
sub configure {
    ...
    if (my $dbtype = $param{dbtype}) {
        ...
        if ($dbtype eq 'mysql') {
            ...
            $param{login_required} = 1;
            $param{publish_charset} = $app->param('publish_charset') || ($app->{cfg}->DefaultLanguage eq 'ja' ? 'Shift_JIS' : 'ISO-8859-1');
    ...
    if ($app->param('test')) {
        ...
        if ($driver) {
            ...
            if (my $err = $@) {
                $err_msg = $app->translate('An error occurred while attempting to connect to the database.  Check the settings and try again.');
                $err_more = $err;
            } else {
                $ok = 1;
            }
...
sub seed {
    ...
    if ($ENV{MOD_PERL} || (($r_uri =~ m/\/mt-wizard\.(\w+)(\?.*)?$/) && ($1 ne 'cgi'))) {
        my $new = '';
        if ($ENV{MOD_PERL}) {
            $param{mod_perl} = 1;
        } else {
