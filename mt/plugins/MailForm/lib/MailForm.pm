...
sub post
{
    ...
    my $email   = $app->{query}->param('mail_email');
    $email      = &_encode_ajax($email, $ajax);
    my $email_confirm = $app->{query}->param('mail_email_confirm');
    $email_confirm    = &_encode_ajax($email_confirm, $ajax);
    my $author  = $app->{query}->param('mail_author');
    $author     = &_encode_ajax($author, $ajax);
    my $subject = $app->{query}->param('mail_subject');
    ...
    for my $param (@params) {
        ...
        $ext_params_tmp = &_encode_ajax($app->{query}->param($param), $ajax);
    ...
    for my $must_check_field (keys %must_check_fields) {
        if (!defined($app->{query}->param($must_check_field))) {
            $iserror = 1;
            $is_input_error = 1;
    ...
    if (!$iserror && !$preview) {
        ...
        if ($reply_template) {
            ...
            $reply_body = $tmpl->build($ctx, \%cond);
        ...
        my $reply_subject = $rstmpl->build($ctx, \%cond);
