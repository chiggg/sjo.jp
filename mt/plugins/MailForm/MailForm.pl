...
my $plugin = MT::Plugin::MailForm->new({
    ...
    doc_link => 'http://www.h-fj.com/blog/mtplgdoc/mailformv2.php',
...
sub mail_form_ajax_js
{
    ...
    my $out = <<HERE;
<script type="text/javascript" src="${static_path}plugins/MailForm/js/prototype.js"></script>
...
sub include_mail_form_common {
    ...
    my $out = $ctx->_hdlr_include($args, $cond);
    $out;
}
